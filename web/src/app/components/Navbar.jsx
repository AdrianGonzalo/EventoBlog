      ...
      <nav className="flex justify-between items-center p-4 max-w-7xl mx-auto">
        ...
        <div className="flex gap-8 text-gray-700 font-medium relative">
          ...
          <div className="relative group cursor-default select-none">
            ...
            {/* Barra rosa debajo del dropdown */}
            <span
              className={`
      absolute -bottom-5 -left-[22px] h-0.5 z-1 bg-pink-600 transition-all
      ${pathname.startsWith("/peinados") ? "w-28" : "w-0 group-hover:w-28"}
    `}
            ></span>

            {/* Dropdown pegado */}
            <div className="absolute left-1/2 -translate-x-1/2 top-8 mt-2 w-28 bg-pink-50/90 border border-pink-100 border-t-0 shadow-lg overflow-hidden transform transition-all duration-200 ease-out origin-top scale-95 opacity-0 pointer-events-none group-hover:scale-100 group-hover:opacity-100 group-hover:pointer-events-auto before:content-[''] before:absolute before:-top-3 before:left-0 before:w-full before:h-3">
              <Link
